# ==============================================
# ETF SECTOR SCREENER EOD - V3 (INDEX PROFILE)
...
# ==============================================
# PARAMETRI PATTERN — profilo "Indexes / Futures"
# ==============================================
...
tickers = [
    "CIBR","IHF","JETS","IAI","IYG","IBB","KBE","QABA","XLF","KIE",
    "KRE","KCE","SMH","PJP","SKYY","IYT","REZ","PEJ","TINY","XRT",
    "XLV","XLK","IGV","FDN","EVX","XHB","INDS","NXTG","SEA","SOCL",
    "VNQ","XLI","ITA","IYK","AMLP","PHO","XLY","BIZD","IYC","PSP",
    "XLP","XLU","XLC","WOOD","IGF","CARZ","MOO","MORT","PBJ","SLX",
    "IHI","XLB","XLE","XOP","GRID","FMAT","HAP","ERTH","IYZ","GNR",
    "FAN","RNRG","COPX","PICK","FCG","COAL","TAN","ICLN","BITB","OIH",
    "XME","LIT","GDX","SIL","NLR","URA",
]
print(f"Universo: {len(tickers)} ETF settoriali/tematici.")
...
def detect_double_top(df):
    ...
    for j in range(len(pivots) - 1, -1, -1):
        ...
        for i in range(j - 1, -1, -1):
            ...
            if abs(t1_val - t2_val) > PEAK_TOL_ATR * atr_ref:
                continue
            ...
            if (hi_top - neckline) < MIN_VALLEY_ATR * atr_ref:
                continue
            ...
            if USE_APEX or USE_TREND:
                ...
                if USE_APEX and t1_val < prior_hi:
                    continue
                if USE_TREND:
                    if (t1_val - prior_lo) < TREND_RISE_ATR * atr_ref:
                        continue
...
results = []
print("Avvio analisi ETF (profilo Indexes/Futures)...")
